...
public class SlideViewPager extends ViewGroup {
    ...
    private final float mOriginalX = 0.0f;
    ...
    private final boolean mIsBeingDragged = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mDownX = event.getRawX();
                Log.d(TAG, " current distance == " + distance);
    ...
    private void performDrag(int distance) {
        ...
        Log.d(TAG, " perform drag distance == " + distance);
        scrollBy(distance, 0);
        if (distance < 0) {
            dragScaleShrinkView(mCurrentPosition, LEFT_TO_RIGHT);
        } else {
            Log.d(TAG, " current direction is right to left and current child position =  " + mCurrentPosition);
...
//        }
    ...
    private void smoothScrollToDes() {
        ...
        Log.d(TAG, " smooth scroll to des position == before =" + mCurrentPosition
...
//        Log.d(TAG, " smooth scroll to des position == " + position + " dx = " + dx + " scroll x == " + scrollX);
...
//        invalidate();
    ...
    public void computeScroll() {
        if (!mScroller.isFinished() && mScroller.computeScrollOffset()) {
            ...
            Log.d(TAG, " compute scroll dx == " + dx + " position == " + mCurrentPosition);
//                int position = mCurrentPosition;
//                if (mCurrentDir == RIGHT_TO_LEFT){
//                    Log.d(TAG, " current direction == right to left" );
//                    position = mCurrentPosition - 1;
//                }else if (mCurrentDir == LEFT_TO_RIGHT){
//                    Log.d(TAG, " current direction == left to right " );
//                    position = mCurrentPosition + 1;
//                }
...
//            }
        ...
        if (distance > 0) {
            ...
            if (direction == LEFT_TO_RIGHT) {//value may be from X to 0
                if (position >= 0) {
                    ...
                    Log.d(TAG, " current scale ratio = " + scaleRatio + " shrink ratio = " + shrinkRatio + " ratio = " + ratio);
                }
    ...
    private void confirmScaleRatio(int width, float gutterSize) {
        ...
        //Log.d(TAG, " confirm scale ratio == " + gutterSize + " ration ==  " + SCALE_RATIO + " margin lef t == " + mMarginLeftRight);
