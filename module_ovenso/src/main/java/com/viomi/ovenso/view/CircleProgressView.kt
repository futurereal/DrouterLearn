...
open class CircleProgressView : View {
     ...
     * 边框圆圈的内层背景色
     */
    private var circleInnerReleaseColor = 0
    private var circleInnerPressColor = 0
    ...
    private var circleOutHintColor = Color.parseColor("#3328beca")

    private lateinit var circleInnerPaint: Paint
    private lateinit var circleOuterPaint: Paint
    private lateinit var circleOuterHintPaint: Paint
    ...
    private fun initAttrSet(attrs: AttributeSet?) {
        ...
        typedArray?.let {
            ...
            //内圆的颜色
            circleInnerReleaseColor =
                it.getColor(R.styleable.circle_progressview_inner_release_color, 0)
            circleInnerPressColor =
                it.getColor(R.styleable.circle_progressview_inner_press_color, 0)
            //内圆风格
            circleInnerStyle =
    ...
    private fun initData() {
        ...
        circleInnerPaint.color = circleInnerReleaseColor
        ...
        // 外圆背景
        circleOuterHintPaint = Paint(circleOuterPaint)
        circleOuterHintPaint.color = circleOutHintColor
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val radius = width / 2 - 20f
        // 内部的圆环
        Log.i(TAG, "onDraw: draw inner circle $radius")
        canvas.drawCircle(mCenterX, mCenterY, radius, circleInnerPaint)
        // 外部的圆环背景
        canvas.drawArc(progressOuterRectF, MAX_DEGREE, MAX_DEGREE, false, circleOuterHintPaint)
        // 外部的圆环
        Log.i(TAG, "onDraw: draw outer circle $circleOutVisible")
        if (circleOutVisible) {
            val sweepAngle = currentProgress * MAX_DEGREE / DEFAULT_MAX
            canvas.drawArc(progressOuterRectF, MAX_DEGREE, sweepAngle, false, circleOuterPaint)
        }
        canvas.restore()
        Log.i(TAG, "onDraw: centerTipDrawableId: $centerTipDrawableId")
        var currentDrawable = centerTipDrawableId
        if (currentProgress >= 90f) {
            currentDrawable = resources.getDrawable(R.drawable.unlock, null)
        }
        if (currentDrawable != null) {
            // 画中心的图片
            val bitmapDrawable = currentDrawable as BitmapDrawable
            val bitmap = bitmapDrawable.bitmap
            val left = mCenterX - bitmap.width / 2
            val top = mCenterY - bitmap.height / 2
        ...
        if (!TextUtils.isEmpty(textTip)) {
            ...
            if (currentDrawable != null) {
                val bitmapDrawable = currentDrawable as BitmapDrawable
                val bitmap = bitmapDrawable.bitmap
                mPoint.y = mPoint.y + bitmap.height / 2 + 15
            }
            Log.d(TAG, "onDraw: text : x: ${mPoint.x}  y: ${mPoint.y}")
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event?.action) {
            MotionEvent.ACTION_DOWN -> {
                textTip = "解锁中..."
                circleInnerPaint.color = circleInnerPressColor
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        Log.i(TAG, "onMeasure: ")
        val width = measureSpec(widthMeasureSpec)
        val height = measureSpec(heightMeasureSpec)
