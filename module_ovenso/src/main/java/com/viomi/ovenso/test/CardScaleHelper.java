...
public class CardScaleHelper {
    ...
    private void initWidth() {
        mRecyclerView.post(new Runnable() {
            ...
            public void run() {
                ...
                mCardWidth = mCardGalleryWidth - WheelUtils.dip2px(mContext, 2 * (mPagePadding + mShowLeftCardWidth));
    ...
    private void computeCurrentItemPos() {
        ...
        boolean pageChanged = Math.abs(mCurrentItemOffset - mCurrentItemPos * mOnePageWidth) >= mOnePageWidth;
        // 滑动超过一页说明已翻页
        if (pageChanged) {
