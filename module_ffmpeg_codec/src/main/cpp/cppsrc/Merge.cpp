...
void Merge::MergeTwo(string srcPath1,string srcPath2,string dstPath)
{
    ...
    */
    ...
    if ((ret = avformat_open_input(&in_fmt1,srcPath1.c_str(),NULL,NULL)) < 0) {
        LOGD("MergeTwo avformat_open_input in_fmt1 fail %s",av_err2str(ret));
        return;
    }
    if (avformat_find_stream_info(in_fmt1, NULL) < 0) {
        LOGD("MergeTwo avformat_find_stream_info infmt1 fail");
        releasesources();
        return;
    }
    LOGD("MergeTwo getIndex ");
    // 将源文件1音视频索引信息找出来
    ...
    LOGD("MergeTwo begain SteamCopy ");
    ...
    while(av_read_frame(in_fmt1,in_pkt1) >=0) {
        ...
        if (in_pkt1->stream_index == video1_in_index) {
            ...
            LOGD("pkt1 video %lld(%6s) du %lld(%s)",in_pkt1->pts,av_ts2timestr(in_pkt1->pts,&dststream->time_base),in_pkt1->duration,av_ts2timestr(in_pkt1->duration,&dststream->time_base));
        }
        ...
        if (in_pkt1->stream_index == audio1_in_index) {
            ...
            LOGD("pkt1 audio %lld(%6s) du%lld(%s)",in_pkt1->pts,av_ts2timestr(in_pkt1->pts,&dststream->time_base),in_pkt1->duration,av_ts2timestr(in_pkt1->duration,&dststream->time_base));
        }
    ...
    while(true) {
        ...
        if (in_pkt2->stream_index == audio2_in_index) {
            ...
            LOGD("pkt2 audio %lld(%s)",in_pkt2->pts,av_ts2timestr(in_pkt2->pts,&dststream->time_base));
            // 更正目标流的索引
    ...
    LOGD("mergeTwo 文件写入完毕...  %s ",dstPath.c_str());
    // 释放资源
    releasesources();
}
...
void Merge::doConvert(AVFrame **dst, AVFrame *src,bool isVideo)
{
    ...
    if (!isVideo && (src->sample_rate != (*dst)->sample_rate || src->format != (*dst)->format || src->channel_layout != (*dst)->channel_layout)) {
        ...
        LOGD("next_audio_pts %lld",next_audio_pts);
...
//            LOGD("ret %s",av_err2str(ret));
        ...
        LOGD("%s pts %lld(%s) dts %lld(%s)",pkt->stream_index == video_ou_index ? "video":"audio",pkt->pts,av_ts2timestr(pkt->pts,&stream->time_base),pkt->pts,av_ts2timestr(pkt->dts,&stream->time_base));
...
void Merge::addMusic(string srcpath,string srcpath2,string dstpath,string st)
{
    ...
    while (av_read_frame(in_fmt1, v_pkt) >= 0) {
        if (v_pkt->stream_index == video1_in_index) {   // 说明是视频
            ...
            LOGD("video pts %lld(%s)",v_pkt->pts,av_ts2timestr(v_pkt->pts, &stream->time_base));
    ...
    while (av_read_frame(in_fmt2, a_pkt) >= 0) {
        if (a_pkt->stream_index == audio1_in_index) {   // 音频
            ...
            LOGD("audio pts %lld(%s)",a_pkt->pts,av_ts2timestr(a_pkt->pts, &a_stream->time_base));
            // 加入音频的时长不能超过视频的总时长
