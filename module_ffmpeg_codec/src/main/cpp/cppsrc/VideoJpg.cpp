...
void
VideoJPG::doJpgGet(string srcPath, string imagDstPath, string start, bool getMore, int pictureNum) {
    LOGD("VideoJPG : doJpgGet");
    if (!getMore) {
        pictureNum = 1;
    }
    int video_index = -1;
    // 要截取的时刻
    int64_t start_pts = stoi(start.substr(0, 2))*3600;
    start_pts += stoi(start.substr(3, 2))*60;
    start_pts += stoi(start.substr(6, 2));
    LOGD("doJpgGet : start_pts %lld",start_pts);
    //打开源文件，存在 AvFomatContext   in 类型 里面   libavformat
    if (avformat_open_input(&in_fmt, srcPath.c_str(), NULL, NULL) < 0) {
        LOGD("doJpgGet avformat_open_input fail");
        return;
    } else {
        LOGD("doJpgGet avformat_open_input success");
    }
     // 获取流文件
    if (avformat_find_stream_info(in_fmt, NULL) < 0) {
        LOGD("doJpgGet avformat_find_stream_info fail");
        return;
    } else {
        LOGD("doJpgGet avformat_find_stream_info success");
    }

    // 遍历 流文件  找出视频索引 video_index
    for (int i = 0; i < in_fmt->nb_streams; i++) {
        // AVStream 是 avfomat 的内部类
        AVStream *stream = in_fmt->streams[i];
        // 加了一个日志就好了
        LOGD("doJpgGet  strem type  ");
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {   // 说明是视频
            video_index = i;
            // 初始化解码器用于解码
            LOGD("doJpgGet begainDecoder");
            AVCodec *codec = avcodec_find_decoder(stream->codecpar->codec_id);
            // 通过codec 获取 AVCodecContext
            de_ctx = avcodec_alloc_context3(codec);
            if (!de_ctx) {
                LOGD("doJpgGet video avcodec_alloc_context3 fail");
                releaseSources();
                return;
            }
            // 设置解码参数，这里直接从源视频流中拷贝
            if (avcodec_parameters_to_context(de_ctx, stream->codecpar) < 0) {
                LOGD("doJpgGet video avcodec_parameters_to_context");
                releaseSources();
                return;
            }
            // 初始化解码器上下文 打开
            if (avcodec_open2(de_ctx, codec, NULL) < 0) {
                LOGD("doJpgGet video avcodec_open2() fail");
                releaseSources();
                return;
            }
            break;
    ...
    // 编码器，主要操作 AVStream
    LOGD("doJpgGet avcodec_find_encoder");
    AVCodec *codec = avcodec_find_encoder(AV_CODEC_ID_MJPEG);
    en_ctx = avcodec_alloc_context3(codec);
    if (!en_ctx) {
        LOGD("doJpgGet avcodec_alloc_context3 fail");
        releaseSources();
        return;
    }
    // 设置编码参数
    AVStream *in_stream = in_fmt->streams[video_index];
    // 编码的参数 context 放在 context 里面
    ...
    if (avcodec_open2(en_ctx, codec, NULL) < 0) {
        LOGD("doJpgGet avcodec_open2() fail");
        releaseSources();
        return;
    }
    ...
    // 创建用于输出JPG的封装器
    // 把路径存在AvForamtContext 里面，还有其他参数？
    LOGD("doJpgGet getImage");
    if (avformat_alloc_output_context2(&ou_fmt, NULL, NULL, imagDstPath.c_str()) < 0) {
        LOGD("doJpgGet avformat_alloc_output_context2");
        releaseSources();
        return;
    }
    ...
    // 设置流参数；直接从编码器拷贝参数   编码器一开始就初始化
    if (avcodec_parameters_from_context(stream->codecpar, en_ctx) < 0) {
        LOGD("doJpgGet avcodec_parameters_from_context");
        releaseSources();
        return;
    }
     ...
     *  AVFormatContext 编码器的context
     */
    if (!(ou_fmt->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open2(&ou_fmt->pb, imagDstPath.c_str(), AVIO_FLAG_WRITE, NULL, NULL) < 0) {
            LOGD("doJpgGet avio_open2 fail");
            releaseSources();
            return;
        }
    }
    ...
    if (avformat_write_header(ou_fmt, NULL) < 0) {
        LOGD("doJpgGet avformat_write_header() fail");
        releaseSources();
        return;
    }
     ...
     */
    sws_ctx = sws_getContext(in_stream->codecpar->width, in_stream->codecpar->height,
                             (enum AVPixelFormat) in_stream->codecpar->format,
    ...
    if (av_frame_get_buffer(en_frame, 0) < 0) {
        LOGD("doJpgGet av_frame_get_buffer fail");
        releaseSources();
        return;
    }
    // 保证可写
    ...
    LOGD("av_seek_frame start_pts %'lld",start_pts );
    ...
    while (av_read_frame(in_fmt, in_pkt) == 0) {
        ...
        // 先解码  把packet 封装到 AvCodecContext 里面
        avcodec_send_packet(de_ctx, in_pkt);
        LOGD("doJpgGet video pts %lld(%s)", in_pkt->pts, av_ts2timestr(in_pkt->pts, &in_stream->time_base));
        while (true) {
             // AVCodecContext 里面获取 AvPackt ，转换为 AVFrame 原始数据
             ...
             */
            LOGD("doJpgGet sucess pts %lld", de_frame->pts);
            // 成功解码出来了
            // 取多帧视频并写入到文件
            static int i = 0;
            LOGD("doJpgGet delt: %lld",delt);
            //偏移量
            delt = delt * pictureNum;
            // 取一帧帧视频并写入到文件
            // 和起始的偏移量的差，小于总的
            if (abs(de_frame->pts - start_pts) < delt) {
                ...
                // 解码的 AVFrame 到 编码的 AVFrame
                ret = sws_scale(sws_ctx, de_frame->data, de_frame->linesize, 0, de_frame->height,
                                en_frame->data, en_frame->linesize);
                if (ret < 0) {
                    LOGD("sws_scale fail");
                    releaseSources();
                    return;
                ...
                LOGD("doJpgGet sws_scale getMore: " + getMore);
                if (getMore) {
                    // 重新编码
                    en_frame->pts = i;
                    i++;
                ...
                LOGD("doJpgGet v_write_frame: begain");
                // 写入文件 AVPackt 写入到 avFormatContext 之前封装的路径里面
                if (av_write_frame(ou_fmt, ou_pkt) < 0) {
                    LOGD("av_write_frame fail");
                    releaseSources();
                    return;
                } else {
                    LOGD("av_write_frame success: ");
                }
...
void VideoJPG::doEncode(AVFrame *en_frame1) {
    ...
    while (true) {
        ...
        LOGD("video pts %lld(%s)", ou_pkt->pts, av_ts2timestr(ou_pkt->pts, &stream->time_base));
