...
void Cut::doWrite(AVPacket *pkt) {
    ...
    if (!has_writer_header) {
        ...
        if (video_en_ctx == NULL) { //  说明没有进行过编码
            ...
            if (avcodec_parameters_copy(au_stream->codecpar,
                                        in_fmtctx->streams[video_in_stream_index]->codecpar) < 0) {
        ...
        if (avcodec_parameters_copy(au_stream->codecpar,
                                    in_fmtctx->streams[audio_in_stream_index]->codecpar) < 0) {
            LOGD("avcodec_parameters_copy fail");
            releasesources();
            return;
        }
    ...
    if (pkt->stream_index == video_ou_tream_index) {
        ...
        if (video_en_ctx != NULL) {
            // 写入之前重新转换一下时间戳
            av_packet_rescale_ts(pkt, video_en_ctx->time_base,
                                 ou_fmtctx->streams[video_ou_tream_index]->time_base);
        } else {
            av_packet_rescale_ts(pkt, in_fmtctx->streams[video_in_stream_index]->time_base,
                                 ou_fmtctx->streams[video_ou_tream_index]->time_base);
        }

    } else {
        // 进行时间戳的转换
        av_packet_rescale_ts(pkt, in_fmtctx->streams[audio_in_stream_index]->time_base,
                             ou_fmtctx->streams[audio_ou_stream_index]->time_base);
    }

    LOGD("%s pts %lld(%s)", pkt->stream_index == video_ou_tream_index ? "vi" : "au", pkt->pts,
         av_ts2timestr(pkt->pts, &ou_fmtctx->streams[pkt->stream_index]->time_base));
...
void Cut::doEncode(AVFrame *enfram) {
    ...
    if (enfram) {
        ...
        video_en_frame->pts =
                video_next_pts * video_en_ctx->time_base.den / video_en_ctx->framerate.num;
        video_next_pts++;
    ...
    while (true) {
        ...
        // 编码成功;写入文件
        LOGD("encode pts %lld(%s)", pkt->pts,
             av_ts2timestr(pkt->pts, &(in_fmtctx->streams[video_in_stream_index]->time_base)));
...
void Cut::doCut(string spath, string cutDestPath, int start_pos, int duration) {
    LOGD("doCut begainCut %s ");
    // 只考虑同一种容器的内容剪切
    srcPath = spath;
    dstPath = cutDestPath;
    if (duration <= 0) {
    ...
    if (video_in_stream_index == -1) {
        LOGD("video_in_stream_index  -1  not have video");
        return;
    }
    AVStream *stream = in_fmtctx->streams[video_in_stream_index];
    ...
    /** 移动当前读取指针到指定的位置
       ...
       */
    if ((ret = av_seek_frame(in_fmtctx, video_in_stream_index,
                             start_pos * stream->time_base.den / stream->time_base.num,
    ...
    if (audio_in_stream_index != -1) {
        ...
        if ((ret = av_seek_frame(in_fmtctx, audio_in_stream_index,
                                 start_pos * stream->time_base.den / stream->time_base.num,
                                 AVSEEK_FLAG_BACKWARD) < 0)) {
    ...
    while (av_read_frame(in_fmtctx, in_packt) >= 0) {
        ...
        if (in_packt->stream_index == video_in_stream_index) {
            ...
            // 允许有5帧的时间差
            int64_t max_frame = 5;
            int64_t delt_pts = av_rescale_q(max_frame,
                                            (AVRational) {1, stream->r_frame_rate.num},
                                            stream->time_base);
            int64_t end_pts =(start_pos + duration) * stream->time_base.den / stream->time_base.num;
            long num = start_pos * stream->time_base.den / stream->time_base.num;
            video_start_pts = (long long) num;

            if (first_video_packet ) {
                if ((video_start_pts - in_packt->pts) <= delt_pts) {
                    LOGD("doCut video_start_pts = %lld ",video_start_pts);
                    video_need_decode = false;
                    LOGD("doCut is key frame pts %s", av_ts2timestr(in_packt->pts, &stream->time_base));
                }
            }
        ...
        if (in_packt->stream_index == audio_in_stream_index && flag) {
            ...
            int64_t end_pts =
                    (start_pos + duration) * stream->time_base.den / stream->time_base.num;
            if (end_pts <= in_packt->pts) {
                audio_end = true;
                continue;;
            }
    ...
    LOGD("doCut  结束写入文件.. %s",cutDestPath.c_str());
    // 释放资源
    releasesources();
}
